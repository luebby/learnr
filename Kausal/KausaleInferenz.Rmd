...
Dieses Tutorial orientiert sich am Projekt [Causal Inference in Introductory Statistics Courses](https://github.com/kfcaby/causalLab) von [LTC Kevin Cummiskey](https://westpoint.edu/mathematical-sciences/profile/kevin_cummiskey).

Datengrundlage ist der Artikel Kahn, M. (2005). *An exhalent problem for teaching statistics*. Journal of Statistics Education, 13(2), [https://doi.org/10.1080/10691898.2005.11910559](https://doi.org/10.1080/10691898.2005.11910559).
...
Rauchen *scheint* dass Lungenvolumen zu erhöhen.
...
question("Sollte über die Variable `Groesse` adjustiert werden?",
  answer("Ja.", message = "Falsch: die Variable `Groesse` liegt auf den kausalen Pfad (*Chain*) zwischen `RaucherIn` und `Lungenvolumen` (Mediator)"),
  answer("Nein.", correct = TRUE, message = "Richtig: die Variable `Groesse` liegt auf den kausalen Pfad (*Chain*) zwischen `RaucherIn` und `Lungenvolumen` (Mediator).")
)
```
...
```{r pwert, echo=FALSE}
question("Ist der beobachtete Effekt ($\\hat{\\beta}_{\\text{RaucherInja}}=-0.153974$) plausibel, wenn es gar keinen geben würde ($H_0: {\\beta}_{\\text{RaucherInja}}=0$)?",
...
*Hinweis*: Die Annahme eines linearen Modells ist aufgrund des nicht-linearen Zusammenhangs zwischen Alter und Größe sowie Größe und Lungenvolumen (siehe Abbildungen in *Das Alter ...*) eher nur eine Näherung.
