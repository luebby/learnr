...
Und natürlich gibt es einen Zusammenhang zwischen der Größe und dem Lungenvolumen:
...
Weil die nicht-kausalen Pfade durch die *Hintertür* (engl. backdoor) offen sind:
...
question("Sollte über die Variable `Groesse` adjustiert werden?",
  answer("Ja.", message = "Falsch: die Variable `Groesse` liegt auf den kausalen Pfad zwischen `RaucherIn` und `Lungenvolumen` (Mediator)"),
  answer("Nein.", correct = TRUE, message = "Richtig: die Variable `Groesse` liegt auf den kausalen Pfad zwischen `RaucherIn` und `Lungenvolumen` (Mediator).")
)
```
