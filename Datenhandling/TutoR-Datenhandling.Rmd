...
In diesem Tutorium lernen Sie ein wenig über exponentielles Denken (was uns leider sehr schwer fällt) und darüber, wie Daten einfach dargestellt und vorverarbeitet werden können.
...
Zusammen mit der Information: Sie sehen den fiktiven Verlauf des Beginns einer Epidemie. Während es zu Beginn 1000 Infizierte gibt, sind es nach 5 Wochen fast 7600.
...
Was die Teilnehmer\*innen nicht wussten: Hinter der Abbildung liegt ein Modell mit $b=1.5$ ($I_0=1000$ konnten Sie dem Text und der Abbildung entnehmen.) In `R` also:
...
Nun können wir uns einen Überblick über die Struktur der Daten (`str()`):
...
```{r Skala, echo=FALSE}
question("Wie viele matrische Variablen liegen vor?",
...
Vergleichen wir die Mittelwerte der studentischen Schätzungen, $\tilde{I}(6)=`r round(mean( ~Woche6, data = EpiEst),2)`$ bzw. $\tilde{I}(7)=`r round(mean( ~Woche7, data = EpiEst),2)`$, mit den wahren Werten, $I(6)=1000 \cdot 1.5^6 = `r 1000*1.5^6`$ und $I(7)=1000 \cdot 1.5^7 = `r 1000*1.5^7`$, grafisch:
...
question("Was sind hier die beobchteten Untersuchungseinheiten?",
...
**Hinweise**: Falls Sie Beobachtungen von weitern Analysen ausschließen, muss dies immer begründet und dokumentiert werden.

Welche Vergleichsoperation (`==` (gleich), `!=` (ungleich), `<=` (kleiner gleich), `>=` (größer gleich)) sollte gewählt werden, damit Sie nur für die Beobachtungen des Datensatzes `EpiEst` aus der Kohorte `A` die Kennzahlen berechnen?
...
Über `mutate()` können Variablen verändert oder neu erzeigt werden:
...
Betrachten wir die prozentuale Abweichung zwischen den wahren Wert $I(t)$ und der (individuellen) Prognose $\tilde{I}(t)_i$:
...
```{r mutate-solution}
EpiEst <- EpiEst %>%
  mutate(Abw.Woche6 = Woche6/11390.62 - 1) %>%
  mutate(Abw.Woche7 = Woche7/17085.94 - 1)
```
...
EpiEst %>%
  summarise(Abw.Woche6 = mean(Abw.Woche6), Abw.Woche7=mean(Abw.Woche7))
```
...
Über `case_when()` können neue Variablen je nach Ausprägung, d.h. bedingt, erzeugt (`mutate()` werden):

![](https://github.com/allisonhorst/stats-illustrations/raw/master/rstats-artwork/dplyr_case_when.png){width=80%}

Abbildung von [\@allisonhorst](https://github.com/allisonhorst/stats-illustrations).

Im folgenden Code wird eine neue Variable (`Überschätzt`) erzeugt. Diese nimmt den Wert `Ja` an, wenn $\frac{\tilde{I}(6)_i}{I(6)} - 1 > 0$ ist, sonst den Wert `Nein`.

```{r case-when, exercise = TRUE, message = FALSE, exercise.setup = "mutate-solution"}
EpiEst %>%
  mutate(Überschätzt = case_when(Abw.Woche6 > 0 ~ "Ja",
                                 Abw.Woche6 <= 0 ~ "Nein")) %>%
  group_by(Überschätzt) %>%
  summarise(Anzahl = n())
```

Wenn mehrere Bedingungen kombiniert werden sollen kann dies z.B. über ein (logisches) *und* (mathematisch $\land$, in R `&`), d.h. z.B. beide Bedingungen müssen erfüllt sein, oder über ein *oder* (mathematisch $\land$, in R `|`), d.h. nur eine Bedingung muss erfüllt sein.
...
EpiEst %>%
  mutate(Überschätzt = case_when((Abw.Woche6 > 0) ___ (Abw.Woche7 > 0) ~ "Ja",
                                 (Abw.Woche6 <= 0) ___ (Abw.Woche7 <= 0) ~ "Nein")) %>%
  group_by(Überschätzt) %>%
  summarise(Anzahl = n())
```
...
EpiEst %>%
  mutate(Überschätzt = case_when((Abw.Woche6 > 0) & (Abw.Woche7 > 0) ~ "Ja",
                                 (Abw.Woche6 <= 0) | (Abw.Woche7 <= 0) ~ "Nein")) %>%
  group_by(Überschätzt) %>%
  summarise(Anzahl = n())
```
...
Die Meisten unterschätzen also den Infektionsverlauf für die Wochen 6 und 7.

##

Da viele Verläufe exponentiell sind, d.h., die Daten ändern sich um einen konstanten Faktor (z.B. Anzahl verdoppelt sich jeden Monat), nicht um eine konstante Summe (z.B. jeden Monat kommen 1000 neue Fälle dazu), ist das Problem leider universell und weit verbreitet. Probieren Sie daher bitte daran zu denken: unsere Intuition kann exponentielles Wachstum schlecht einschätzen, wir unterschätzen es. Oder hätten Sie gedacht, dass die Antwort auf die Frage, wie oft Sie ein Papier falten müssten (sofern dies beliebig oft gefaltet werden könnte), damit die Dicke bis zum Mond reicht, 42 ist?
