...
Die Funktion `xpnorm(q, mean = 0, sd = 1)` berechnet für eine Normalverteilung mit Mittelwert `mean` und Standardabweichung `sd` den Wert der Verteilungsfunktion an der der Stelle `q`, d.h., $p=F(q)$. 
...
Während `p <- xpnorm(q, mean = 0, sd = 1)` die Wahrscheinlichkeit `p` (bei bekannten $\mu$ und $\sigma$) für eine Beobachtung kleiner gleich `q` berechnet, ergibt `q <- xqnorm(p, mean = 0, sd = 1)` den Wert `q` zurück, der mit einer Wahrscheinlichkeit `p` nicht überschritten wird -- jeweils bei Normalverteilung:
