...
In diesem Tutorial sollen Sie lernen, wie Sie zwei Gruppen vergleichen können (*A/B Testing*). Unsere interessierende Variable `Y` wird modelliert duch eine Variable `X`, wobei `X` genau zwei Werte annehmen kann: `Y ~ X`.

Wie können Sie induktiv die Daten einer Stichprobe nutzen? 

## Nachhaltigkeit und betriebswirtschaftlicher Erfolg

Nachhaltigkeit von Unternehmen ist ein altes und immer noch aktuelles Thema (CSR: Corporate Social Responsibility). Und, was glauben Sie? lohnt es sich auch finanziell für die Unternehmen (CFP: Corporate Financial Performance)? Was glauben Sie?

###

### Wissenschaft

*Glauben* oder *meinen* sind unter wissenschaftlichen Gesichtspunkten vielleicht keine guten Argumentationsgrundlagen. 

### Literatur

Suchen Sie doch mal, z.B., unter [https://scholar.google.de](https://scholar.google.de), nach Literatur zu dem Thema.
...
Eine Meta-Analye, Friede, G., Busch, T., & Bassen, A. (2015). *ESG and financial performance: aggregated evidence from more than 2000 empirical studies*. Journal of Sustainable Finance & Investment, 5(4), 210-233. [https://doi.org/10.1080/20430795.2015.1118917](https://doi.org/10.1080/20430795.2015.1118917) schreibt:
...
Zu Vorbereitung installiertes Paket `mosaic` laden:
...
- `report`: Gab es im Geschäftsbericht des Unternehmens im Jahre 2008 einen Nachhaltigkeitsbericht? (`X`)
...
question("Welches Skalenniveau hat die Variable `report`?",
  answer("Kategorial - nominal", correct=TRUE,  message = "Richtig, die Werte (`Ja`, `Nein` können nur unterschieden werden. Bei zwei Ausprägungen spricht man auch von *binären* oder *dichotomen* Variablen."),
  answer("Kategorial - ordinal", message = "Falsch: die Werte können unterschieden werden, es gibt aber keine natürliche Ordnung."), 
    answer("Numerisch - intervall", message = "Falsch: die Werte können unterschieden werden, es gibt aber keine natürliche Ordnung. Auch können evtl. Abstände nicht verglichen werden."), 
    answer("Numerisch - verhältnis", message = "Falsch: die Werte können unterschieden werden, es gibt aber keine natürliche Ordnung. Auch können evtl. Abstände nicht verglichen werden und es gibt keinen absoluten Nullpunkt.")
)
```
...
Bei *Beobachtungsstudien* (wie hier) kann man sich zunächst *nicht* sicher sein, ob es nicht z.B. Kovariablen gibt, die den Zusammenhang beeinflussen. Wie bei der Korrelation zwischen Störchen und Geburten.

Bei *randomisierten Experimenten* wird den Beobachtungen *zufällig* eine sogenannte "Behandlung" zugewiesen. Dadurch hofft man, dass sich diese gleichmäßig verteilen. Mögliche Kovariablen sind hier z.B. Geschäftsfeld, Unternehmensgröße etc.. Bei der Frage CSR und CFP ist z.B. vorstellbar, dass besonders gut dastehende Unternehmen sich einen Nachhaltigkeitsbereicht "leisten können".
...
*Hinweis*: Um den un-gruppierten, allgemeinen, unbedingten Mittelwert für die Variable `roa` zu bekommen können Sie die erklärende Variable ($x$, hier `report`) durch `1` ersetzen:
...
Auch wenn die Gesamtkapitalrentabilität am Ende des Jahres fest steht, so sind doch verschiedene Realisierungen möglich (gewesen), so dass sie als *zufällig* aufgefasst werden kann. Durch unternehmerische Entscheidungen hofft man gleichwohl zurecht, dass man Einfluss nehmen kann.
...
Um die *Verteilung* des Mittelwertes zu schätzen, kann (wieder) Bootstrapping, d.h., resamplen der Stichprobe, verwendet werden:
...
question("Stimmt die Aussage: 95% der Beobachtungen von `roa` liegen zwischen 0.044 und 0.074?",
...
```{r beweis, echo=FALSE}
question("Beweist die Analyse, dass $\\mu_{Nein}-\\mu_{Ja} <0 \\Leftrightarrow \\mu_{Nein}<\\mu_{Ja}$ gilt?",
...
Beachte, dass ein Mittelwert von $0$ vom Konfidenzintervall überdeckt wird. In $2.5\, \%$ der Resamples beträgt die Differenz des Mittelwertes von `roa` maximal `r confint(Bootvtlg.diffmean)$lower` (ist also $\leq$), in $2.5\, \%$ der Resamples ist sie mindestens `r confint(Bootvtlg.diffmean)$lower` (ist also $\geq$). 


## Hypothesenprüfung

**Wenn** es *keinen* Unterschied in der Verteilung Gesamtkapitalrentabilität zwischen den Unternehmen mit und ohne Nachhaltigkeitsbericht gibt, dann können wir diese *permutieren*. Außerdem gilt unter der **Annahme** *es gibt keinen Unterschied*, dann gilt auch

\[H_0: \mu_{Nein}-\mu_{Ja} = 0 \Leftrightarrow \mu_{Nein}=\mu_{Ja}\]

Die Verteilung der Differenz der Mittelwerte von `roa`, wenn wir die Werte von `report` vertauschen können, können wir simulieren. In `mosaic` erfolgt dieses permutieren über den Befehl `shuffle()`. Drücken Sie ruhig öfter auf `Run Code` -- die beobachtete Differenz variiert auch unter dem Modell der Nullhypothese:

```{r shuffle, excercise=TRUE}
...
Nachdem Sie der Variable $abw.stipro$ den Wert `r diffmean(roa ~ report, data = CsrCfp)` zugewiesen haben können Sie diese auch verwenden.
...
Der **p-Wert** sagt aus, wie häufig man *unter den Annahmen der Nullhypothese*, hier also der Gleichheit der Verteilungen, man eine mindestens so große Abweichung in einer Stichprobe sieht. 
...
question("Stimmt die Aussage: Die Wahrscheinlichkeit, dass $H_0$ stimmt, liegt bei $0.384$.",
...
Unter den Annahmen einer Normalverteilung wird eine solche Hypothese klassisch über einen t-Test, Funktion `t.test()` geprüft. Setzen Sie den Befehl entsprechend ein.

```{r ttest, excersise = TRUE, eval = FALSE}
___(roa ~ report, data = CsrCfp)
```
...
Das Paket [dabestr](https://cran.r-project.org/package=dabestr) bietet eine konfortable Möglichkeit die Daten samt Konfidenzintervall zu visualisieren. Siehe auch Ho et al. (2018) [https://doi.org/10.1101/377978](https://doi.org/10.1101/377978).
...
Kausale Inferenz jenseits eines hier nicht möglichen randomisierten Experiments kann über *Directed Acyclic Graphs* oder auch über das *Potential Outcome* Konzept analysiert werden.
