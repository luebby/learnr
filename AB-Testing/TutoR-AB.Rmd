...
Um solchen Fehlinterpretationen möglichst vorzubeugen wird, in der Hoffnung, dass sich die Daten gleichmäßig verteilen, in *randomisierten Experimenten* den Beobachtungen *zufällig* eine sogenannte "Behandlung" zugewiesen. 
...
Was, wenn wir `roa` als (allgemeines) lineares Modell von `report` darstellen? Also
...
$${roa}_i=`r coef(erglm)[1]`  `r coef(erglm)[2]` \cdot  \begin{cases}1, \,\text{i hat keinen Report veröffentlicht} \\ 0, \,\text{i hat einen Report veröffentlicht}\end{cases} + \epsilon_i $$
