...
p_wert <- prop( ~ (abs.abw.null >= abs.abw.stipro), data = Nullvtlg)
...
$$\bar{y}_{Ja}=`r mean(roa ~ report, data = CsrCfp)[1]`>`r mean(roa ~ report, data = CsrCfp)[2]`=\bar{y}_{Nein}$$
...
$$\hat{\mu}=\bar{y}=`r mean(~roa, data = CsrCfp)`$$


### Konfidenzintervall 

Um die *Verteilung* des Mittelwertes zu schätzen kann Bootstrapping, d. h. Re-Sampeln der Stichprobe, verwendet werden:
...
`diffmean()` berechnet direkt die Differenz, d. h. $\bar{y}_{Nein}-\bar{y}_{Ja}$.
...
Die in der Stichprobe beobachtete Differenz von $\bar{y}_{Nein}-\bar{y}_{Ja}$ liegt bei:
...
question(paste("Stimmt die Aussage: Die Wahrscheinlichkeit, dass $H_0$ stimmt, liegt bei ", p_wert, "."),
...
Wenn Sie mehr als zwei Gruppen vergleichen wollen, gehen einfache Differenzen (`diffmean()` bzw. `diffprop()`) nicht mehr. Hier helfen dann z. B. die $\chi^2$-Statistik (kategorial, `xchisq.test()`) oder $F$-Statistik (metrisch, `aov()`) weiter.
