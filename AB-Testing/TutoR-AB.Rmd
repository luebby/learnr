...
In diesem Tutorial sollen Sie lernen, wie Sie zwei Gruppen vergleichen können (*A/B Testing*). Wie können Sie induktiv die Daten einer Stichprobe nutzen? 
...
- `report`: Gab es im Geschäftsbericht des Unternehmens im Jahre 2008 einen Nachhaltigkeitsbericht?
- `roa`: Gesamtkapitalrentabilität (Return on Assets).
...
**Wenn** es *keinen* Unterschied in der Verteilung Gesamtkapitalrentabilität zwischen den Unternehmen mit und ohne Nachhaltigkeitsbericht gibt, dann können wir diese *permutieren*. Außerdem gilt unter der **Annahme** *es gibt keinen Unterschied*: 

\[H_0: \mu_{Nein}-\mu_{Ja} = 0 \Leftrightarrow \mu_{Nein}=\mu_{Ja}\]

Die Verteilung für die Differenz der Mittelwerte von `roa`, wenn wir die Werte von `report` vertauschen können, können wir simulieren. In `mosaic` erfolgt dieses permutieren über den Befehl `shuffle()`. Drücken Sie ruhig öfter auf `Run Code` -- die beobachtete Differenz variiert auch unter dem Modell der Nullhypothese:
...
do(10) * t.test(roa ~ report, data = resample(CsrCfp)) %>%
  pval()
```
