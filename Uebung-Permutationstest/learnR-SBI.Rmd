...
Sei $F_{\text{Bier}}(y)$ die Verteilung der Anzahl Mücken beim Biertringen und $F_{\text{Wasser}}(y)$ die Anzahl Mücken beim Wassertrinken.
...
`gf_jitter(mein.y ~ mein.x, data = Meine.Daten)` zeichnet ein *verwackeltes* Streudiagramm. Dies bietet sich hier an, da beide Variablen diskret sind. Welche Form der Darstellung bietet sich an, d.h. welche Variable (`mosquitoes`, `drink`) sollte auf welche Achse?
